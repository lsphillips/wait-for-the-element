...
/**
 * Fetches an element by waiting for it to not exist.
 *
 * Example usage:
 *
 * ``` js
 * try
 * {
 *     let element = await waitForTheElementToHide('.selector-for-an-element-that-may-appear-later', {
 *         timeout : 5000
 *     });
 * }
 * catch (error)
 * {
 *     throw new Error('Took more than 5 seconds to find the element gone.');
 * }
 * ```
 *
 * By default it will search the entire document and wait for 2.5 seconds.
 ...
 * @typeParam E The type of the element you expect to be found.
 *
 * @returns A promise that will resolve with an element matching `selector`, or reject with an error if an element is still found in time.
 */
export function waitForTheElementToHide<E extends Element> (selector : string, options? : ElementSearchOptions) : Promise<E>;

// - - - - - - - - - - - - - - - - - - - - - - - - - - - -

/**
 * Fetches an element by waiting for it to not exist, but returns `null` instead of throwing an error.
 *
 * Example usage:
 *
 * ``` js
 * let element = await tryAndWaitForTheElementToHide('.selector-for-an-element-that-may-appear-later', {
 *     timeout : 5000
 * });
 *
 * if (element === null)
 * {
 *     console.log('Took more than 5 seconds to find the element gone.');
 * }
 * ```
 *
 * By default it will search the entire document and wait for 2.5 seconds.
 ...
 * @typeParam E The type of the element you expect to be found.
 *
 * @returns A promise that will resolve with an element matching `selector`, or `null` if an element is still found in time.
 */
export function tryAndWaitForTheElementToHide<E extends Element> (selector : string, options? : ElementSearchOptions) : Promise<E | null>;